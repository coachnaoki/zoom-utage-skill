...
2. Claude Code を起動する（`claude` コマンド）

3. 以下の文章をそのまま貼り付けて送信する
   ```
...
4. API 認証情報を準備する（取得手順は **X記事（配布元）** を参照）
   - **Zoom API**（Server-to-Server OAuth）: Account ID / Client ID / Client Secret → X記事 STEP1
   - **Gemini API キー**: → X記事 STEP2
   - **UTAGE オペレーターアカウント**と3つのURL: → X記事 STEP3

5. API キー・URL は `create_lesson.py` 実行時に **対話形式で聞かれ、自動で `.env` に保存**される。DIYで `.env` を編集する必要はない。

6. ターゲット層に合わせて色・フォント・Gemini プロンプトを調整する場合
   ```
...
初回実行時、APIキー・UTAGE ログイン・3つのURL（login/course/upload-folder）は対話入力され、自動的に `.env` に保存される。2回目以降は引数なしで動く。

### A. Zoom APIモード（推奨）

```bash
.venv/bin/python create_lesson.py --zoom --zoom-days 7
```

直近7日の Zoom 録画一覧（MP4+VTTが揃ったもの）を表示し、**番号を入力して選択**する。デフォルトは最新（1番）。`--yes` を付けると確認なしで最新を自動選択。
...
.venv/bin/python create_lesson.py \
  --vtt "/path/to/GMT20260413-110040_Recording.transcript.vtt" --yes
```
...
別コース・別アップロード先で使う時は引数で上書き可能:

```bash
.venv/bin/python create_lesson.py --zoom --yes \
  --course-url "https://utage-system.com/site/SITE_ID/course/OTHER_COURSE_ID"
```
...
| フラグ | 用途 |
|---|---|
| `--slides-url URL` | 「資料を開く」ボタンを追加（未指定時は実行時に対話で毎回聞かれる。不要ならEnter） |
